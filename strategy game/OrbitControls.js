 ...
 *
 * @author victorsh /
 * 	-Modified for the purposes of Strategy Game
 *	-Pan has been modified to work excusively on the xz plane
 *	
...
THREE.OrbitControls = function ( object, domElement, localElement ) {
	...
	this.panLeft = function ( distance ) {
		...
		panOffset.set( te[0], 0, te[2] );//panOffset.set( te[0], te[1], te[2] );
		panOffset.multiplyScalar(-distance);

		var newX = this.target.x + panOffset.x;
		var newZ = this.target.z + panOffset.z;

		if(newX <= this.maxPanX && newX >= -this.maxPanX && newZ <= this.maxPanZ && newZ >= -this.maxPanZ){
			pan.add( panOffset );
		}
	};
	...
	this.panUp = function ( distance ) {
		...
		panOffset.set( te[4], 0, te[6] );//panOffset.set( te[4], te[5], te[6] );
		panOffset.multiplyScalar(distance);
